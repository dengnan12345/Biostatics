...
x<-as.matrix(read.table(file="p53.1130sgRNA_100bp_6mer.txt"))
...
PCC.Lasso.6mer.top20<-cor(y.test.total[sgRNA.top20.id],y.test.pred.top20)
PCC.Lasso.6mer.all <- cor(y.test.total,y.test.pred.total.Lasso.mean)
save(y.test.total,y.test.pred.total.Lasso.mean,PCC.Lasso.6mer.top20,PCC.Lasso.6mer.all,file="PCC.Lasso.6mer.Rdata")
...
PCC.Ridge.6mer.top20<-cor(y.test.total[sgRNA.top20.id],y.test.pred.top20)
PCC.Ridge.6mer.all <- cor(y.test.total,y.test.pred.total.Ridge.mean)
save(y.test.total,y.test.pred.total.Ridge.mean,PCC.Ridge.6mer.top20,PCC.Ridge.6mer.all,file="PCC.Ridge.6mer.Rdata")


##Perform RandomForest
library(randomForest)
PCC.RF.6mer.100bp.top20<-matrix(,10,50)
PCC.RF.6mer.100bp.all<-matrix(,10,50)
rownames(PCC.RF.6mer.100bp.top20)<-rep("ntree",10)
rownames(PCC.RF.6mer.100bp.all)<-rep("ntree",10)
for(n in 1:10){
	rownames(PCC.RF.6mer.100bp.top20)[n]=paste("ntree=",50+50*(n-1),sep="")
	rownames(PCC.RF.6mer.100bp.all)[n]=paste("ntree=",50+50*(n-1),sep="")
}
colnames(PCC.RF.6mer.100bp.top20)<-rep("mtry",50)
colnames(PCC.RF.6mer.100bp.all)<-rep("mtry",50)
for(m in 1:50){
	colnames(PCC.RF.6mer.100bp.top20)[m]=paste("mtry=",10+10*(m-1),sep="")
	colnames(PCC.RF.6mer.100bp.all)[m]=paste("mtry=",10+10*(m-1),sep="")
}
...
for(n in 1:10){
	...
	for(m in 1:50){
		...
		PCC.RF.6mer.100bp.top20[n,m]<-cor(y.test.total[sgRNA.top20.id],y.test.pred.RF.total[[n]][[m]][sgRNA.top20.id])
		PCC.RF.6mer.100bp.all[n,m]<-cor(y.test.total,y.test.pred.RF.total[[n]][[m]])
		}
	}
##Perform CV to determine optimal parameters of ntree and mtry
save(PCC.RF.6mer.100bp.top20,PCC.RF.6mer.100bp.all,y.test.total,y.test.pred.RF.total,file="PCC.RF.6mer.100bp.50ntree.Rdata")
...
save(PCC.SVR.RBF.top20,PCC.SVR.RBF.all,y.test.pred.SVR.total,y.test.total,file="SVR_RBF_PCC_6mer_100bp.Rdata")
